...
- 1000 字五份全部單字（各 217 字，共 1085 張字卡）在解釋旁提供對應意思的 AI 圖片；正面與測驗不顯示圖片，2000 字書維持原樣。圖片只表達其中一個意思，不取代完整解釋；預載目前及接下來 10 字的可用圖片，語音與換字不等待圖片。圖片讀取失敗時恢復純文字，字卡高度不變
...
Part 1、Part 2、Part 3 新增的 641 筆配圖對應及共用原因見 [Part 1–3 配圖記錄](docs/word-illustrations-parts1-3.json)。原先完成的 444 筆與圖檔保留不變。執行 `node scripts/import-word-illustrations.mjs --parts1-3` 可由該記錄重建前端對應表；完整檢查要求 1085 張字卡全數有圖，且 2000 字書沒有被帶入配圖。補齊圖片不增加整本預載、動畫或連播等待條件。
